...
class medoo
{
	...
	public function exec($query, $keep_mode = false)
	{
		if ($this->debug_mode)
		{
			echo $query . "\n";
	...
	{
		preg_match('/(\(JSON\)\s*|^#)?([a-zA-Z0-9_]*)\.([a-zA-Z0-9_]*)/', $table, $table_match);

		if (isset($table_match[ 2 ], $table_match[ 3 ]))
		{
			return '`' . $table_match[ 2 ] . '`.`' . $table_match[ 3 ] . '`';
		}
...
//			echo "type of $value is $type\n";
	...
	protected function where_clause($where)
	{
		...
		if (is_array($where))
		{
			...
			if (isset($where[ 'GROUP' ]))
			{
				...
				{
					foreach ($where[ 'GROUP' ] as $k => $v)
					{
						$where[ 'GROUP' ][$k] = $this->column_quote($where[ 'GROUP' ][$k]);
					}
	...
	public function insert($table, $datas)
	{
		...
		foreach ($datas as $data)
		{
			...
			$this->exec('INSERT INTO ' . $this->table_quote($table) . ' (' . implode(', ', $columns) . ') VALUES (' . implode($values, ', ') . ')', true);
	...
	public function multi_insert($table, $datas, $schema, $group = 200)
	{
		$lastId = array();
		...
		if (!isset($schema) || !$schema || !count($schema))
		{
			$columns = "";
		}
		else
	...
	public function update($table, $data, $where = null)
	{
		...
		foreach ($data as $key => $value)
		{
			...
			if (isset($match[ 3 ]))
			{
				//if (is_numeric($value))     // 表达式也可以相加减 modified by zzy 2016-7-14
				{
...
//				echo "type of $value is " . gettype($value) . "\n";
	...
	public function action($actions, $params = NULL)
	{
