			...
			foreach (['team1', 'team2'] as $aside) {
				...
				foreach (['A', 'B'] as $aplayer) {
					...
					if ($last == "Qualifier") {
						...
						if (substr($uuid, 0, 3) == "atp" || strpos($amatch['eventName'], 'Men\'s') !== false || strpos($amatch['eventName'], 'Boy') !== false) {
							$gender = "M";
						} else {
							$gender = "F";
						}
...
*/
	...
	protected function parseSchedule($day) {
		...
		foreach ($json['courts'] as $court) {
			...
			if (!$start_time) {
				$start_time = strtotime($date . " 23:00");
			}
			$next_time = 0;
			...
			foreach ($court['matches'] as $amatch) {
				...
				if (strpos($courtName, 'Arthur Ashe') !== false) {
					$courtId = 1;
				} else if (strpos($courtName, 'Louis Armstrong') !== false) {
					$courtId = 2;
				} else if (strpos($courtName, 'Grandstand') !== false) {
					$courtId = 3;
				} else if ($courtName == 'Court 17') {
					$courtId = 4;
				} else if ($courtName == 'Court 5') {
					$courtId = 5;
				} else if ($courtName == 'Court 10') {
					$courtId = 6;
				} else if ($courtName == 'Court 13') {
					$courtId = 7;
