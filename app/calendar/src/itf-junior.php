...
class Calendar extends CalendarBase {
	...
	protected function download() {
		if ($this->mode != "local") {
			$this->url = "https://www.itftennis.com/Umbraco/Api/TournamentApi/GetCalendar?circuitCode=JT&searchString=&skip=0&take=100&nationCodes=&zoneCodes=&dateFrom=$this->start&dateTo=$this->end&indoorOutdoor=&categories=&isOrderAscending=true&orderField=startDate&surfaceCodes=";
			$content = http($this->url, null, $this->cookie2, null);
			if ($content == "") return [false, "download failed"];
			$this->content = $content;
		} else {
			$this->content = file_get_contents("itf_junior_calendar");
		}
		return [true, ""];
	}
