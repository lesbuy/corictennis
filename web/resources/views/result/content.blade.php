...
$(function() {
	$('#iResult{{ $tour[0] }} .cResultMatchTime').each(function () {
...
@foreach ($tour[6] as $courtName => $matches)
	<div class=cResultCourt court-id="{{ strtolower(preg_replace('/^.*\t|[\' \"-]/', '', $courtName)) }}">
		<div class=cResultCourtTitle>{{ explode("\t", $courtName)[1] }}</div>
		@foreach ($matches as $match)
			<div class="cResultMatch {{ $match[24] }} {{ $match[20] }}" match-id="{{ $match[0] }}" best-of="{{ $match[17] }}" match-status="{{ $match[16] }}" is-double="{{ $match[19] }}" data-bets-id="{{ @$match[28] }}" data-odds="{{ @$match[29] }}">
				...
				<div class=cResultMatchMid>
					...
					<table>
						<tr class="cResultMatchMidTableRowOdd cResultMatchMidTableRow{{ $match[10] }}">
							<td>
								{!! $match[5] !!}{!! isset($match[29]) && $match[29] && !App::isLocale('xx') ? "<odds class=cResultMatchOdds>" . explode("-", $match[29])[0] . "</odds>" : "" !!}
								<div>
						...
						<tr class="cResultMatchMidTableRow{{ $match[11] }}">
							<td>
								{!! $match[6] !!}{!! isset($match[29]) && $match[29] && !App::isLocale('xx') ? "<odds class=cResultMatchOdds>" . explode("-", $match[29])[1] . "</odds>" : "" !!} 
								<div>
